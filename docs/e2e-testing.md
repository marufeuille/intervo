...
- タイマーを開始したテストはサービスを残さないこと。`@Before` で
  `startService(Intent(...).setAction(TimerService.ACTION_STOP))` を投げ、`runningWorkoutId` のリークで
  次のテストが自動再開ダイアログに飛ぶのを防いでいる。**テストが中断前に失敗するとサービスが残り、
  以後の実行（同じエミュレータ）で自動再開して連鎖失敗するため、新しいタイマー操作シナリオを足すときは
  必ず末尾で中断して落ちないよう注意する。**
...
- 休憩/レップのスキップ、フリーセット記録（スキップボタンはアイコンのみなので `testTag` を付ける等が必要。
  また休憩へ到達させるには 1 種目目を短くする＝種目フェーズが一瞬になるため、次種目を長くして滞在させる設計が要る）
